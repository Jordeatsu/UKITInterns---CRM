...
function submit({ name, email, phone, subject, description, complaint_type_id, product_ids = [] }) {
  ...
  const complaintType = complaint_type_id
    ? db.prepare('SELECT priority FROM complaint_types WHERE id = ?').get(complaint_type_id)
    : null;
  ...
  const transaction = db.transaction(() => {
    ...
    if (!contact) {
      ...
      ).run(name, normalizedEmail, phone || null);
    ...
    const insertCaseProduct = db.prepare(
      'INSERT INTO case_products (case_id, product_id) VALUES (?, ?)'
    );
...
function update(id, { status, assigned_to, priority, changedBy = 'system' }) {
  ...
  const insertHistory = db.prepare(
    'INSERT INTO case_history (case_id, changed_by, field, old_value, new_value) VALUES (?, ?, ?, ?, ?)'
  );
  const recordChanges = db.transaction(() => {
    if (newStatus !== existing.status)
      insertHistory.run(id, changedBy, 'status', existing.status, newStatus);
    if (newAssignedTo !== existing.assigned_to)
      insertHistory.run(id, changedBy, 'assigned_to', existing.assigned_to, newAssignedTo);
    if (newPriority !== existing.priority)
      insertHistory.run(id, changedBy, 'priority', existing.priority, newPriority);
...
function addCommentCode(caseId, productId, commentCodeId) {
  let caseProduct = db.prepare(
    'SELECT * FROM case_products WHERE case_id = ? AND product_id = ?'
  ).get(caseId, productId);

  if (!caseProduct) {
    const ins = db.prepare(
      'INSERT INTO case_products (case_id, product_id) VALUES (?, ?)'
    ).run(caseId, productId);
...
function addMessage(caseId, senderType, senderName, content) {
  const result = db.prepare(
    'INSERT INTO case_messages (case_id, sender_type, sender_name, content) VALUES (?, ?, ?, ?)'
  ).run(caseId, senderType, senderName, content);
...
function getHistory(caseId) {
  return db.prepare(
    'SELECT * FROM case_history WHERE case_id = ? ORDER BY changed_at DESC'
  ).all(caseId);
}
